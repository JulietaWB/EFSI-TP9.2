      ...
      <HoverScale>
        <div className="card p-8">
          ...
          <div className="mt-6 grid grid-cols-3 text-sm">
            ...
            <div className="flex items-center gap-2 justify-end text-gray-200"><WiThermometerExterior className="text-accent" />S.T. {Math.round(currentWeather.main.feels_like)}{unitLabel}</div>
          </div>
          <div className="mt-4 flex items-center justify-between text-sm text-gray-300">
            <div>Mín: {Math.round(currentWeather.main.temp_min)}{unitLabel}</div>
            <div>Máx: {Math.round(currentWeather.main.temp_max)}{unitLabel}</div>
          </div>
        </div>
      </HoverScale>
    </FadeIn>
